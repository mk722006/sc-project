def local_report(month, year, selected_country, table_name, conn, curr):
    ...
    file_name = f"Executive Dashboard {selected_country} {convert_month_year(month, year).split('-')[0]} {year} {datetime.now().strftime('%Y-%m-%d')}.xlsx"
    ...
    copy_wb = xw.Book(copy_file_path)
    ...
    def spend_percentage_value_function(dictionary_data, column_for_name, column_spend, column_percentage, copy_wb, cell_number, sheet_name, run):
        ...
        if column_for_name!=False: ## Set to False when you do not have column_for_name
            ...
            copy_wb.sheets(sheet_name).range(cell_name_A).value = cell_value_spend
        ...
        if column_spend!= False:
            ...
            copy_wb.sheets(sheet_name).range(cell_name_B).value = round(cell_value_spend, 2)
        ...
        if column_percentage!= False:
            ...
            copy_wb.sheets(sheet_name).range(cell_name_C).value = cell_value_percentage
    ...
    copy_wb.sheets(sheet_name).range('A2').value = f'Global Supply Chain Analytics - {month_year_abbr.split("-")[0]} 2023'


    ## B6, D6 | Ex: CLP Spend MTD
    copy_wb.sheets(sheet_name).range('B6').value = f'{curr_symbol} Spend MTD'
    copy_wb.sheets(sheet_name).range('D6').value = f'{curr_symbol} Spend YTD'

    ## MTD Currency Headers: E47, E98, E153 | Ex: MTD BRL Spend
    mtd_currency_spend_headers = ['E47', 'E98', 'E152']
    for head in mtd_currency_spend_headers:
        copy_wb.sheets(sheet_name).range(head).value = f'MTD {curr_symbol} Spend'
        
    ## YTD Currency Headers: F47, F98, F153 | Ex: YTD BRL Spend
    ytd_currency_spend_headers = ['F47', 'F98', 'F152']
    for head in ytd_currency_spend_headers:
        copy_wb.sheets(sheet_name).range(head).value = f'YTD {curr_symbol} Spend'
    ...
    for head in headers_currency_list:
        copy_wb.sheets(sheet_name).range(head).value = curr_symbol
    ...
    for j, value in enumerate(executive_summary_values):
        cell_value = round(list(value[0].values())[0], 2)
        copy_wb.sheets(sheet_name).range(cell_names_executive_summary[j]).value = cell_value
    ...
    for cell_name in header_list:
        copy_wb.sheets(sheet_name).range(cell_name).value = B20_D20_Header_cell_value
    ...
    for cell in table_header_cells:
        copy_wb.sheets(sheet_name).range(cell).value = table_header_value
    ...
    copy_wb.sheets(sheet_name).range('B42').value = all_others
    copy_wb.sheets(sheet_name).range('C42').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(top_manufacturers_by_category_spend_pharma)
    copy_wb.sheets(sheet_name).range('F42').value = all_others
    copy_wb.sheets(sheet_name).range('G42').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(top_manufacturers_by_category_spend_indirect)
    copy_wb.sheets(sheet_name).range('J42').value = all_others
    copy_wb.sheets(sheet_name).range('K42').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('B48').value = f'{month_year_abbr} ( {curr_symbol} )'  # Ex: Apr-23 ( USD )
    copy_wb.sheets(sheet_name).range('B59').value = f"{month_year_abbr.split('-')[0]} YTD {curr_symbol}"  # Ex: Apr YTD USD 

    # Writing in Cells - D59, E59, F59, G59 | Headers 
    copy_wb.sheets(sheet_name).range('D59').value = three_months_prior[-1]  # Jan-23
    copy_wb.sheets(sheet_name).range('E59').value = two_months_prior[-1]  # Feb-23
    copy_wb.sheets(sheet_name).range('F59').value = one_month_prior[-1]  # Mar-23
    copy_wb.sheets(sheet_name).range('G59').value = current_date[-1]  # Apr-23
    ...
    copy_wb.sheets(sheet_name).range('A70').value = f'Top Suppliers {month_year_abbr} MTD'  # Ex: Top Suppliers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('D70').value = f'Top Suppliers {month_year_abbr} YTD'  # Ex: Top Suppliers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('G70').value = f'Top Manufacturers {month_year_abbr} MTD'  # Ex: Top Manufacturers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('J70').value = f'Top Manufacturers {month_year_abbr} YTD'  # Ex: Top Manufacturers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('A85').value = f'Top Product Categories {month_year_abbr} MTD'  # Ex: Top Product Categories  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('G85').value = f'Top Product Categories {month_year_abbr} YTD'  # Ex: Top Product Categories  Apr-23  YTD
    ...
    copy_wb.sheets(sheet_name).range('B82').value = all_others
    copy_wb.sheets(sheet_name).range('C82').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(direct_non_pharma_spend_top_suppliers_value_ytd) # | Top Suppliers
    copy_wb.sheets(sheet_name).range('E82').value = all_others
    copy_wb.sheets(sheet_name).range('F82').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('H82').value = all_others
    copy_wb.sheets(sheet_name).range('I82').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('K82').value = all_others
    copy_wb.sheets(sheet_name).range('L82').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('E91').value = all_others
    copy_wb.sheets(sheet_name).range('F91').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(direct_non_pharma_spend_top_product_categories_value_ytd) 
    copy_wb.sheets(sheet_name).range('K91').value = all_others
    copy_wb.sheets(sheet_name).range('L91').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('B99').value = f'{month_year_abbr} ( {curr_symbol} )'  # Ex: Apr-23 ( USD )
    copy_wb.sheets(sheet_name).range('B110').value = f"{month_year_abbr.split('-')[0]} YTD {curr_symbol}"  # Ex: Apr YTD USD 

    # Writing in Cells - D110, E110, F110, G110 | Headers 
    copy_wb.sheets(sheet_name).range('D110').value = three_months_prior[-1]  # Jan-23
    copy_wb.sheets(sheet_name).range('E110').value = two_months_prior[-1]  # Feb-23
    copy_wb.sheets(sheet_name).range('F110').value = one_month_prior[-1]  # Mar-23
    copy_wb.sheets(sheet_name).range('G110').value = current_date[-1]  # Apr-23
    ...
    # Writing in Cells - A70, D70, G70, J70, A85, G85 | Headers | Suppliers, Manufacturers
    copy_wb.sheets(sheet_name).range('A123').value = f'Top Suppliers {month_year_abbr} MTD'  # Ex: Top Suppliers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('D123').value = f'Top Suppliers {month_year_abbr} YTD'  # Ex: Top Suppliers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('G123').value = f'Top Manufacturers {month_year_abbr} MTD'  # Ex: Top Manufacturers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('J123').value = f'Top Manufacturers {month_year_abbr} YTD'  # Ex: Top Manufacturers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('A139').value = f'Top Product Categories {month_year_abbr} MTD'  # Ex: Top Product Categories  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('G139').value = f'Top Product Categories {month_year_abbr} YTD'  # Ex: Top Product Categories  Apr-23  YTD
    ...
    copy_wb.sheets(sheet_name).range('B135').value = all_others
    copy_wb.sheets(sheet_name).range('C135').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(direct_pharma_spend_top_suppliers_value_ytd) # | Top Suppliers
    copy_wb.sheets(sheet_name).range('E135').value = all_others
    copy_wb.sheets(sheet_name).range('F135').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('H135').value = all_others
    copy_wb.sheets(sheet_name).range('I135').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('K135').value = all_others
    copy_wb.sheets(sheet_name).range('L135').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('E145').value = all_others
    copy_wb.sheets(sheet_name).range('F145').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(direct_pharma_spend_top_product_categories_value_ytd) 
    copy_wb.sheets(sheet_name).range('K145').value = all_others
    copy_wb.sheets(sheet_name).range('L145').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('B153').value = f'{month_year_abbr} ( {curr_symbol} )'  # Ex: Apr-23 ( USD )
    copy_wb.sheets(sheet_name).range('G153').value = f"{month_year_abbr.split('-')[0]} YTD {curr_symbol}"  # Ex: Apr YTD USD 

    # Writing in Cells - I153, J153, K153, L153 | Headers 
    copy_wb.sheets(sheet_name).range('I153').value = three_months_prior[-1]  # Jan-23
    copy_wb.sheets(sheet_name).range('J153').value = two_months_prior[-1]  # Feb-23
    copy_wb.sheets(sheet_name).range('K153').value = one_month_prior[-1]  # Mar-23
    copy_wb.sheets(sheet_name).range('L153').value = current_date[-1]  # Apr-23
    ...
    copy_wb.sheets(sheet_name).range('A163').value = f'Top Suppliers {month_year_abbr} MTD'  # Ex: Top Suppliers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('D163').value = f'Top Suppliers {month_year_abbr} YTD'  # Ex: Top Suppliers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('G163').value = f'Top Manufacturers {month_year_abbr} MTD'  # Ex: Top Manufacturers  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('J163').value = f'Top Manufacturers {month_year_abbr} YTD'  # Ex: Top Manufacturers  Apr-23  YTD

    copy_wb.sheets(sheet_name).range('A178').value = f'Top Product Categories {month_year_abbr} MTD'  # Ex: Top Product Categories  Apr-23  MTD
    copy_wb.sheets(sheet_name).range('G178').value = f'Top Product Categories {month_year_abbr} YTD'  # Ex: Top Product Categories  Apr-23  YTD
    ...
    copy_wb.sheets(sheet_name).range('B175').value = all_others
    copy_wb.sheets(sheet_name).range('C175').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(indirect_spend_top_suppliers_value_ytd) # | Top Suppliers
    copy_wb.sheets(sheet_name).range('E175').value = all_others
    copy_wb.sheets(sheet_name).range('F175').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(indirect_spend_top_manufacturers_value_mtd) # | Top Manufacturers
    copy_wb.sheets(sheet_name).range('H175').value = all_others
    copy_wb.sheets(sheet_name).range('I175').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(indirect_spend_top_manufacturers_value_ytd) # | Top Manufacturers
    copy_wb.sheets(sheet_name).range('K175').value = all_others
    copy_wb.sheets(sheet_name).range('L175').value = all_others_percentage
    ...
    copy_wb.sheets(sheet_name).range('E184').value = all_others
    copy_wb.sheets(sheet_name).range('F184').value = all_others_percentage

    all_others, all_others_percentage = getting_all_others_info(indirect_spend_top_product_categories_value_ytd) 
    copy_wb.sheets(sheet_name).range('K184').value = all_others
    copy_wb.sheets(sheet_name).range('L184').value = all_others_percentage
    ...
    yoy_date_range = get_date_ranges(2023)
    index_to_add = market_df[market_df['country'] == selected_country].index[0] - 1
    ...
    for dt in yoy_date_range:
        ...
        query = f"""select sum (dollar_total_cost) as MTD_usd_local from {table_name} where date_of_purchase > '{start_yoy_date}' and date_of_purchase < '{end_yoy_date}' and geography = '{selected_country}'"""
        curr.execute(query)
        value = curr.fetchall()
    ...
    for index, name in enumerate(column_names_yoy_table):
        ...
        copy_wb.sheets(sheet_name).range(cell_name).value = cell_value
    ...
    for dt in yoy_date_range:
        start_yoy_date, end_yoy_date = dt
        query = f"""select sum (dollar_total_cost) as MTD_usd_local from {table_name} where date_of_purchase > '{start_yoy_date}' and date_of_purchase < '{end_yoy_date}' and geography = '{selected_country}' and spend_type_1 = 'Direct (Non-Pharma)'"""
        curr.execute(query)
        value = curr.fetchall()
        value = list(value[0].values())[0]
    ...
    for dt in yoy_date_range:
        start_yoy_date, end_yoy_date = dt
        query = f"""select sum (dollar_total_cost) as MTD_usd_local from {table_name} where date_of_purchase > '{start_yoy_date}' and date_of_purchase < '{end_yoy_date}' and geography = '{selected_country}' and spend_type_1 = 'Direct (Pharma)'"""
        curr.execute(query)
        value = curr.fetchall()
        value = list(value[0].values())[0]
    ...
    value = curr.fetchall()
    # print(value)
    copy_wb.sheets(sheet_name).range('A44').value = f"*Total unique mfg {list(value[0].values())[0]}"
    ...
    value = curr.fetchall()
    # print(value)
    copy_wb.sheets(sheet_name).range('E44').value = f"*Total unique mfg {list(value[0].values())[0]}"
    ...
    value = curr.fetchall()
    # print(value)
    copy_wb.sheets(sheet_name).range('I44').value = f"*Total unique mfg {list(value[0].values())[0]}"
